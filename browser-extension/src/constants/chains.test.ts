...
describe("Chain Registry", () => {
  describe("CHAIN_CONFIGS", () => {
    it("should have configuration for Astar", () => {
      ...
      expect(config.dappUrl).toBe("https://dapp.blockchainkeyvault.com");
    ...
    it("should have configuration for Localhost/Hardhat", () => {
      ...
      expect(config.dappUrl).toBe("http://localhost:5173");
    });
    ...
    it("should have valid ChainConfig structure for each chain", () => {
      Object.entries(CHAIN_CONFIGS).forEach(([chainIdStr, config]) => {
        ...
        expect(typeof config.dappUrl).toBe("string");
  ...
  describe("isValidChainId", () => {
    ...
    it("should return false for Polygon (unsupported)", () => {
      expect(isValidChainId(137)).toBe(false);
    });
  });

  describe("Chain ID consistency", () => {
    it("should have matching chain IDs in CHAIN_CONFIGS and viem chains", () => {
      expect(CHAIN_CONFIGS[astar.id].chain.id).toBe(astar.id);
      expect(CHAIN_CONFIGS[base.id].chain.id).toBe(base.id);
      expect(CHAIN_CONFIGS[hardhat.id].chain.id).toBe(hardhat.id);
    });

    it("should have Astar chain ID of 592", () => {
      expect(astar.id).toBe(592);
      expect(CHAIN_CONFIGS[592]).toBeDefined();
    });
